...
import { getMall, listEmployees } from "../graphql/queries";
import { updateEmployee, updateMall } from "../graphql/mutations";
...
export default function MallUpdateForm(props) {
  const {
    id: idProp,
    mall: mallModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = mallRecord
      ? { ...initialValues, ...mallRecord, Employees: linkedEmployees }
      : initialValues;
  ...
  const [mallRecord, setMallRecord] = React.useState(mallModelProp);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      setMallRecord(record);
    };
    queryData();
  }, [idProp, mallModelProp]);
  React.useEffect(resetStateValues, [mallRecord, linkedEmployees]);
  ...
  return (
    <Grid
        ...
        try {
          ...
          employeesToUnLink.forEach((original) => {
            if (!canUnlinkEmployees) {
              throw Error(
                `Employee ${original.id} cannot be unlinked from Mall because mallID is a required field.`
              );
            }
            promises.push(
              client.graphql({
          ...
          promises.push(
            client.graphql({
              query: updateMall.replaceAll("__typename", ""),
              variables: {
                input: {
                  id: mallRecord.id,
                  ...modelFieldsToSave,
      ...
      {...getOverrideProps(overrides, "MallUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mallModelProp)}
